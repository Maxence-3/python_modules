...
def parser(inventory: dict[str, int], args: list[str]) -> bool | None:
    try:
        for arg in args:
...
def least_abundant(
        inventory: dict[str, int], min_quantity: int) -> tuple[str, int]:
    min_item = ""
    for item, quantity in inventory.items():
...
def sort_and_display(inventory: dict[str, int], total: int) -> None:
    sorted = dict()
    ...
    while len(sorted) < len(inventory):
        ...
        print(f"{max_item}: {max_quantity} \
{'unit' if max_quantity == 1 else 'units'} \
({(max_quantity / total * 100):.1f}%)")
...
def restock_info(inventory: dict[str, int]) -> list[str]:
    restock_needed = []
    for item, quantity in inventory.items():
        if quantity == 1:
...
ft_inventory_system.py <itemName>:<itemQuantity> ...")
    else:
        args: list[str] = sys.argv[1:]
        inventory: dict[str, int] = dict()
        if parser(inventory, args):
            total: int = count_items(inventory)
